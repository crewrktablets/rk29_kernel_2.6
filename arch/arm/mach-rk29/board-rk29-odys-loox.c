...
#endif
#define PMEM_GPU_SIZE       SZ_16M// SZ_64M
...
#define LCD_CS_PIN             INVALID_GPIO // RK29_PIN6_PC7// RK29_PIN6_PD1//INVALID_GPIO
/*****************************************************************************************
* frame buffe  devices
* author: zyw@rock-chips.com
*****************************************************************************************/
 ...
 * author: zyw@rock-chips.com / astralix
 *****************************************************************************************/
...
static int rk29_fb_io_init(struct rk29_fb_setting_info *fb_setting)
{
	...
	/* LCD CS -- Chip Select: Mandatory, Output, Default Low */
	ret = gpio_request(RK29_PIN6_PC7, NULL);
	if(ret != 0)
	{
		gpio_free(RK29_PIN6_PC7);
		printk(">>>>>> LCD_CS_PIN gpio_request err \n ");
	}
	gpio_direction_output(RK29_PIN6_PC7, GPIO_LOW);
	gpio_set_value(RK29_PIN6_PC7, GPIO_LOW);

	/* LCD FMK -- ?? : Optional, Output, Default Low */
	...
	/* LCD DON --  Display On: Optional, Output, Default ?? */
	...
	/* LCD STBY --  Display Standby: Optional, Output, Default ?? */
    if(fb_setting->disp_on_en && (FB_LCD_STANDBY_PIN != INVALID_GPIO))
    {
		...
		/* Astralix: Looks like they are messing around with the
		 * backlight here
		 */
        rk29_mux_api_set(GPIO1B5_PWM0_NAME, GPIO1L_GPIO1B5);
        ret = gpio_request(RK29_PIN1_PB5, NULL);
        if(ret != 0)
        {
	...
	/* LCD CABC -- ?? : Optional, Output, Default Low */
...
static struct platform_device rk29_v4l2_output_device = {
	.name		= "rk29_vout",
};


/*****************************************************************************************
 * LZ300MSF Touch Initialization
 * author: zyw@rock-chips.com / astralix
 *****************************************************************************************/
#define TOUCH_INT_PIN   RK29_PIN0_PA2

static int i2c_lz300_init_platform_hw(void)
{
...
static struct i2c_lz300_platform_data i2c_lz300_info = {
	.model= 300,
	.init_platform_hw= i2c_lz300_init_platform_hw,
	//.screen_x[] = { 50, 750,  50, 750, 400},
	//.screen_y[] = { 50,  50, 550, 550, 300},
	//.uncali_x_default[] = { 812,  3341, 851,  3371, 2183 },
	//.uncali_y_default[] = { 3638,  3664, 564,  591, 2087 },
};


/*****************************************************************************************
 * MMA8452 g-Sensor Initialization
 * author: zyw@rock-chips.com / astralix
 *****************************************************************************************/
 ...
 * I2C Bus 0
 */
#ifndef CONFIG_I2C0_RK29
#error "ODYS LOOX needs I2C-0 enabled"
#endif
...
#warning "ODYS LOOX has no devices at I2C-1"
 ...
 * I2C Bus 2
 */
#ifndef CONFIG_I2C2_RK29
#error "ODYS LOOX needs I2C-2 enabled"
#endif
...
#warning "ODYS LOOX has no devices at I2C-3"
...
#define SENSOR_IIC_ADDR_0 	    		0x00	//0x60
...
#define SENSOR_FLASH_PIN_0				INVALID_GPIO
...
/* Front Camera Sensor */
#define SENSOR_NAME_1					RK29_CAM_SENSOR_NAME_OV7675
#define SENSOR_IIC_ADDR_1				0x42
...
#define SENSOR_FLASH_PIN_1				INVALID_GPIO
...
static int rk29_sensor_init( struct rk29camera_platform_data *camp)
{
    int ret = 0;
	struct rk29camera_gpio_res *cgpiop = camp->gpio_res;

	cgpiop->gpio_init = 0;

	if (cgpiop->gpio_power != INVALID_GPIO) {
		ret = gpio_request(cgpiop->gpio_power, "camera power");
		if (ret)
			goto sensor_err_out;
		cgpiop->gpio_init |= RK29_CAM_POWERACTIVE_MASK;
		gpio_set_value(cgpiop->gpio_power, (((~cgpiop->gpio_flag)&RK29_CAM_POWERACTIVE_MASK)>>RK29_CAM_POWERACTIVE_BITPOS));
		gpio_direction_output(cgpiop->gpio_power, (((~cgpiop->gpio_flag)&RK29_CAM_POWERACTIVE_MASK)>>RK29_CAM_POWERACTIVE_BITPOS));

		//printk("\n%s....power pin(%d) init success(0x%x)	\n",__FUNCTION__,camera_power,(((~camera_ioflag)&RK29_CAM_POWERACTIVE_MASK)>>RK29_CAM_POWERACTIVE_BITPOS));
	}

	if (cgpiop->gpio_reset != INVALID_GPIO) {
		ret = gpio_request(cgpiop->gpio_reset, "camera reset");
		if (ret)
			goto sensor_err_out;
		cgpiop->gpio_init |= RK29_CAM_RESETACTIVE_MASK;
		gpio_set_value(cgpiop->gpio_reset, ((cgpiop->gpio_flag&RK29_CAM_RESETACTIVE_MASK)>>RK29_CAM_RESETACTIVE_BITPOS));
		gpio_direction_output(cgpiop->gpio_reset, ((cgpiop->gpio_flag&RK29_CAM_RESETACTIVE_MASK)>>RK29_CAM_RESETACTIVE_BITPOS));

		//printk("\n%s....reset pin(%d) init success(0x%x)\n",__FUNCTION__,camera_reset,((camera_ioflag&RK29_CAM_RESETACTIVE_MASK)>>RK29_CAM_RESETACTIVE_BITPOS));
	}

	if (cgpiop->gpio_powerdown != INVALID_GPIO) {
		ret = gpio_request(cgpiop->gpio_powerdown, "camera powerdown");
		if (ret)
			goto sensor_err_out;
		cgpiop->gpio_init |= RK29_CAM_POWERDNACTIVE_MASK;
		gpio_set_value(cgpiop->gpio_powerdown, ((cgpiop->gpio_flag&RK29_CAM_POWERDNACTIVE_MASK)>>RK29_CAM_POWERDNACTIVE_BITPOS));
		gpio_direction_output(cgpiop->gpio_powerdown, ((cgpiop->gpio_flag&RK29_CAM_POWERDNACTIVE_MASK)>>RK29_CAM_POWERDNACTIVE_BITPOS));

		//printk("\n%s....powerdown pin(%d) init success(0x%x) \n",__FUNCTION__,camera_powerdown,((camera_ioflag&RK29_CAM_POWERDNACTIVE_BITPOS)>>RK29_CAM_POWERDNACTIVE_BITPOS));
	}

	if (cgpiop->gpio_flash != INVALID_GPIO) {
		ret = gpio_request(cgpiop->gpio_flash, "camera flash");
		if (ret)
			goto sensor_err_out;
		cgpiop->gpio_init |= RK29_CAM_FLASHACTIVE_MASK;
		gpio_set_value(cgpiop->gpio_flash, ((cgpiop->gpio_flag&RK29_CAM_FLASHACTIVE_MASK)>>RK29_CAM_FLASHACTIVE_BITPOS));
		gpio_direction_output(cgpiop->gpio_flash, ((cgpiop->gpio_flag&RK29_CAM_FLASHACTIVE_MASK)>>RK29_CAM_FLASHACTIVE_BITPOS));
...
/**
 ** Astralix: using pointers makes the code half in size
 **/
static int rk29_sensor_io_deinit(int sensor)
{
	struct rk29camera_gpio_res *camera_gpios = &rk29_camera_platform_data.gpio_res[sensor];
	unsigned int ginit = camera_gpios->gpio_init;

	if ((ginit & RK29_CAM_POWERACTIVE_MASK) && (camera_gpios->gpio_power != INVALID_GPIO)) {
        gpio_direction_input(camera_gpios->gpio_power);
        gpio_free(camera_gpios->gpio_power);
	}

	if ((ginit & RK29_CAM_RESETACTIVE_MASK) && (camera_gpios->gpio_reset != INVALID_GPIO)) {
        gpio_direction_input(camera_gpios->gpio_reset);
        gpio_free(camera_gpios->gpio_reset);
	}

	if ((ginit & RK29_CAM_POWERDNACTIVE_MASK) && (camera_gpios->gpio_powerdown != INVALID_GPIO)) {
        gpio_direction_input(camera_gpios->gpio_powerdown);
        gpio_free(camera_gpios->gpio_powerdown);
	}

	if ((ginit & RK29_CAM_FLASHACTIVE_MASK) && (camera_gpios->gpio_flash != INVALID_GPIO)) {
        gpio_direction_input(camera_gpios->gpio_flash);
        gpio_free(camera_gpios->gpio_flash);
	}

	camera_gpios->gpio_init = 0;
    return 0;
}
...
static int rk29_sensor_ioctrl(struct device *dev, enum rk29camera_ioctrl_cmd cmd, int on)
{
	...
	struct rk29camera_gpio_res *camera_gpios = NULL;

    if(rk29_camera_platform_data.gpio_res[0].dev_name
		&& (strcmp(rk29_camera_platform_data.gpio_res[0].dev_name, dev_name(dev)) == 0))
	{
		camera_gpios = &rk29_camera_platform_data.gpio_res[0];
		camera_gpios->gpio_init = 0;
    }
	else if (rk29_camera_platform_data.gpio_res[1].dev_name
		&& (strcmp(rk29_camera_platform_data.gpio_res[1].dev_name, dev_name(dev)) == 0))
	{
		camera_gpios = &rk29_camera_platform_data.gpio_res[1];
		camera_gpios->gpio_init = 0;
    }
	else {
 	...
 	switch (cmd)
 	{
 		case Cam_Power:
		{
			if (camera_gpios->gpio_power != INVALID_GPIO)  {
				if (camera_gpios->gpio_init & RK29_CAM_POWERACTIVE_MASK) {
			        if (on) {
			        	gpio_set_value(camera_gpios->gpio_power, ((camera_gpios->gpio_flag & RK29_CAM_POWERACTIVE_MASK) >> RK29_CAM_POWERACTIVE_BITPOS));
						msleep(10);
					} else {
						gpio_set_value(camera_gpios->gpio_power, (((~camera_gpios->gpio_flag) & RK29_CAM_POWERACTIVE_MASK) >> RK29_CAM_POWERACTIVE_BITPOS));
					}
					...
					printk("\n%s..%s..PowerPin=%d request failed!\n", __FUNCTION__, dev_name(dev), camera_gpios->gpio_power);
				}
		    } else {
				ret = RK29_CAM_EIO_INVALID;
		    }
		...
		case Cam_Reset:
		{
			if (camera_gpios->gpio_reset != INVALID_GPIO) {
				if (camera_gpios->gpio_init & RK29_CAM_RESETACTIVE_MASK) {
					if (on) {
			        	gpio_set_value(camera_gpios->gpio_reset, ((camera_gpios->gpio_flag & RK29_CAM_RESETACTIVE_MASK) >> RK29_CAM_RESETACTIVE_BITPOS));
					} else {
						gpio_set_value(camera_gpios->gpio_reset,(((~camera_gpios->gpio_flag) & RK29_CAM_RESETACTIVE_MASK) >> RK29_CAM_RESETACTIVE_BITPOS));
			        }
				} else {
					ret = RK29_CAM_EIO_REQUESTFAIL;
					printk("\n%s..%s..ResetPin=%d request failed!\n", __FUNCTION__, dev_name(dev), camera_gpios->gpio_reset);
				}
		    } else {
				ret = RK29_CAM_EIO_INVALID;
		    }
		...
		case Cam_PowerDown:
		{
			if (camera_gpios->gpio_powerdown != INVALID_GPIO) {
				if (camera_gpios->gpio_init & RK29_CAM_POWERDNACTIVE_MASK) {
					if (on) {
			        	gpio_set_value(camera_gpios->gpio_powerdown, ((camera_gpios->gpio_flag & RK29_CAM_POWERDNACTIVE_MASK) >> RK29_CAM_POWERDNACTIVE_BITPOS));
					} else {
						gpio_set_value(camera_gpios->gpio_powerdown,(((~camera_gpios->gpio_flag) & RK29_CAM_POWERDNACTIVE_MASK) >> RK29_CAM_POWERDNACTIVE_BITPOS));
			        }
				} else {
					ret = RK29_CAM_EIO_REQUESTFAIL;
					printk("\n%s..%s..PowerDownPin=%d request failed!\n", __FUNCTION__, dev_name(dev), camera_gpios->gpio_powerdown);
				}
		    } else {
				ret = RK29_CAM_EIO_INVALID;
		    }
		...
		case Cam_Flash:
		{
			if (camera_gpios->gpio_flash != INVALID_GPIO) {
				if (camera_gpios->gpio_init & RK29_CAM_FLASHACTIVE_MASK) {
                    switch (on)
                    {
                        case Flash_Off:
                            gpio_set_value(camera_gpios->gpio_flash,(((~camera_gpios->gpio_flag) & RK29_CAM_FLASHACTIVE_MASK) >> RK29_CAM_FLASHACTIVE_BITPOS));
		        		    break;
					...
					ret = RK29_CAM_EIO_REQUESTFAIL;
					printk("\n%s..%s..FlashPin=%d request failed!\n", __FUNCTION__, dev_name(dev), camera_gpios->gpio_flash);
				}
		    } else {
				ret = RK29_CAM_EIO_INVALID;
		    }
...
static int rk29_sensor_ioctrl(struct device *dev, enum rk29camera_ioctrl_cmd cmd, int on)
{
    unsigned int camera_power = INVALID_GPIO;
	unsigned int camera_reset = INVALID_GPIO;
	unsigned int camera_powerdown = INVALID_GPIO;
	unsigned int camera_flash = INVALID_GPIO;
	unsigned int camera_ioflag;
	unsigned int camera_io_init;
	int ret = RK29_CAM_IO_SUCCESS;
...
{
	gpio_direction_output(PWM_GPIO, GPIO_HIGH); //added by liqing
    msleep(200);
	gpio_direction_output(PWM_GPIO, GPIO_LOW); //added by liqing
    gpio_free(PWM_GPIO);
	rk29_mux_api_set(PWM_MUX_NAME, PWM_MUX_MODE);
	return 0;
}
...
static int rk29_sdmmc0_cfg_gpio(void)
{
	...
	rk29_mux_api_set(GPIO2A2_SDMMC0DETECTN_NAME, GPIO2L_SDMMC0_DETECT_N);//GPIO2L_GPIO2A2); Modifyed by xbw at 2011-04-25
