...
#define AMAP_RBRC           (0<<14)  /* Address mapping: rank,bank,row,column */
...
/* TPR0..2: SDRAM Timing Parameters Registers */
/* TPR0 */
#define TMRD(n)             ((n)&0x3)
...
// static __sramdata uint32_t Barrier[32];
...
refresh:
...
#if 0
    delayus(1);
    pDDR_Reg->RSLR[0] = 0x249;
    pDDR_Reg->RDGR[0] = 0x55;
    delayus(1);
#else
    delayus(1);
    pDDR_Reg->CCR |= DTT;
    ...
    if((pDDR_Reg->CSR & 0x100000)
        /* add by yk for data byte error after data trainning */
        ||((ddr_freq>=324)&&(ddr_freq <534)&&(pDDR_Reg->RSLR[0]!=0x249))
        )
    {
        //while(ddr_stop);
...
//	uint32_t          rdgr[2];
//	uint32_t          cnt;
//	dtt_cnt_t         dtt_info[4][24];

    ddr_print("version 1.01 20110422 \n");

    mem_type = (pDDR_Reg->DCR & 0x3);
    ddr_type = DDR3_TYPE;		/* dram_type */
    ddr_stop = 1;
...
#if 0
    for(n=400;n>100;n-=10)
    {
        memset(dtt_info, 0x0, sizeof(dtt_info));
        for(value=0;value<1000;value++)
        {
            ...
            while(1)
            {
                rslr_tmp[0]=pDDR_Reg->RSLR[0];
                rslr_tmp[1]=pDDR_Reg->RSLR[0];
                rslr_tmp[2]=pDDR_Reg->RSLR[0];
                rdgr_tmp[0]=pDDR_Reg->RDGR[0];
                rdgr_tmp[1]=pDDR_Reg->RDGR[0];
                rdgr_tmp[2]=pDDR_Reg->RDGR[0];
                if((rslr_tmp[0] == rslr_tmp[1])
                   && (rslr_tmp[1] == rslr_tmp[2])
                   && (rdgr_tmp[0] == rdgr_tmp[1])
                   && (rdgr_tmp[1] == rdgr_tmp[2]))
                {
                    break;
                }
            }
            rslr[0]=rslr_tmp[0];
            rdgr[0]=rdgr_tmp[0];
            printk("CSR:0x%x, RSLR:0x%x, RDGR:0x%x, RSLR_tmp:0x%x, RDGR_tmp:0x%x, \b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b", pDDR_Reg->CSR, pDDR_Reg->RSLR[0], pDDR_Reg->RDGR[0], rslr_tmp[0], rdgr_tmp[0]);
            printk("                                                                                                                               \b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b");
            while(1)
            {
                rslr_tmp[0]=pDDR_Reg->RSLR[0];
                rslr_tmp[1]=pDDR_Reg->RSLR[0];
            ...
            {
                for(row=0;row<(sizeof(rslr)/sizeof(rslr[0]));row++)
                {
                    bw = ((rslr[row]>>(3*addr)) & 0x7)*4;
                    bw += (((rdgr[row]>>(2*addr)) & 0x3)+1);
                    for(cnt=0;cnt<(sizeof(dtt_info)/sizeof(dtt_info[0]));cnt++)
                    {
                        if(dtt_info[addr][cnt].value != 0)
                        {
                          if(dtt_info[addr][cnt].value == bw)
                          {
                              dtt_info[addr][cnt].cnt++;
                              break;
                          }
                        }
                        else
                        {
                          dtt_info[addr][cnt].value = bw;
                          dtt_info[addr][cnt].time = ((bw-2)*100*1000)/(4*n);
                          dtt_info[addr][cnt].cnt =1;
                          break;
                        }
                    }
                }
        ...
        for(addr=0;addr<4;addr++)
        {
            ...
            for(cnt=0;cnt<(sizeof(dtt_info)/sizeof(dtt_info[0]));cnt++)
            {
                ...
                printk("90_clk:%d, delay:%d.%dns, cnt:%d\n", dtt_info[addr][cnt].value, (dtt_info[addr][cnt].time)/100,(dtt_info[addr][cnt].time)%100, dtt_info[addr][cnt].cnt);
                bw = 1;
            }
        }
    }
    ...
    value = ddr_change_freq(DDR_FREQ);//freq
