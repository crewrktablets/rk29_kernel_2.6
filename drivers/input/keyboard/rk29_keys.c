...
static int gsensor_create_file(struct device *pdev);
//JUST FOR HOLD KEY
#define   HOLDPRE    0
#define   HOLDING    1
#define   HOLD_ON_PIN    RK29_PIN1_PD6
struct rk29_hold_key{
     struct timer_list holdtimer;
     int  holdflag;
     struct input_dev *input;
};

static struct rk29_hold_key *rk29_holdkey;
static irqreturn_t set_hold_key_isr(int irq, void *dev_id)
{
      struct rk29_hold_key *holdkey=(struct rk29_hold_key *)dev_id;
      mod_timer(&holdkey->holdtimer,jiffies + msecs_to_jiffies(100));
      return IRQ_HANDLED;
}

static void set_hold_flag_timer(unsigned long _data)
{
     int status;
     struct rk29_hold_key *holdkey=(struct rk29_hold_key *)_data;
     struct input_dev *input = holdkey->input;
     unsigned int type = EV_KEY;

     switch(holdkey->holdflag)
     {
          case HOLDPRE:
	   {
               if(!gpio_get_value(HOLD_ON_PIN)){
                   holdkey->holdflag=HOLDING;
                   input_event(input, type, KEY_F13, 1);
		     input_sync(input);
...
{
	return sprintf(buf, "%d", rk29_holdkey->holdflag);
}

static DEVICE_ATTR(get_gsensor_status, 0777, gsensor_status_show, NULL);
...
static void keys_timer(unsigned long _data)
{
	...
	if(button->gpio != INVALID_GPIO)
		...
		state = !!button->adc_state;
        if((button->gpio==RK29_PIN6_PA7)&&state){
                   if(powerofftimer++>AUTO_POWEROFF_CNT){
                      input_event(input, type, KEY_HP, 1);
                      input_sync(input); 
                      input_event(input, type, KEY_HP, 0);
                      input_sync(input);
                      powerofftimer=0;
                      return;
                   }
...
static int __devinit keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
        ...
        input_set_capability(input, EV_KEY, KEY_HP);//¿¿¿¿¿¿¿¿?
       /**************ÉèÖÃHOLD°´¼ü**************/
	holdkey=kzalloc(sizeof(struct rk29_hold_key),GFP_KERNEL);
       error = gpio_request(HOLD_ON_PIN, "hold_key");
       if (error < 0) {
               pr_err("gpio-keys: failed to request GPIO %d\n",HOLD_ON_PIN);
               gpio_free(HOLD_ON_PIN);
       }

       error = gpio_direction_input(HOLD_ON_PIN);
       if (error < 0) {
           pr_err("gpio-keys: failed to configure input"
	...
	irq = gpio_to_irq(HOLD_ON_PIN);
      ...
      error = request_irq(irq, set_hold_key_isr,IRQF_TRIGGER_FALLING , "hold_key",holdkey);
      if (error) {
              pr_err("gpio-keys: Unable to claim irq %d; error %d\n",
              irq, error);
              gpio_free(HOLD_ON_PIN);
      }
      setup_timer(&holdkey->holdtimer,set_hold_flag_timer,(unsigned long)holdkey);
      holdkey->holdflag=!gpio_get_value(HOLD_ON_PIN);
      rk29_holdkey=holdkey;
      rk29_holdkey->input=input;
...
static int keys_suspend(struct device *dev)
{
	...
	int i;
       powerofftimer=0;
       del_timer(&rk29_holdkey->holdtimer);
...
static int keys_resume(struct device *dev)
{
	...
	struct rk29_keys_platform_data *pdata = pdev->dev.platform_data;
	int i;
	struct input_dev *input = rk29_holdkey->input;
       unsigned int type = EV_KEY;
	if (device_may_wakeup(&pdev->dev)) {
		for (i = 0; i < pdata->nbuttons; i++) {
			struct rk29_keys_button *button = &pdata->buttons[i];
       ...
       if(rk29_holdkey->holdflag)
	  mod_timer(&rk29_holdkey->holdtimer,jiffies + msecs_to_jiffies(7000));
       powerofftimer=0;
	return 0;
}
...
{
       int error;
	error=device_create_file(pdev, &dev_attr_get_gsensor_status);
	if(error<0){
           printk("yyz__________key create file gsensor node failed");
	}
	return error;
}
