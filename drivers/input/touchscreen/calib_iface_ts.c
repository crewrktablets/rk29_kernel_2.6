...
int uncali_x_default[5] = { 378,  3694, 378,  3694, 2036 };
int uncali_y_default[5] = { 3471, 3471, 482,   482, 1977 };
int uncali_x[5] = { 0 };
int uncali_y[5] = { 0 };
int uncali_limit_x = 0;
int uncali_limit_y = 0;
...
extern s8 Init_LZ300MSF(struct i2c_client *client,s16 *xlimits,s16 *ylimits);
extern s16 X_LIMITS ;
extern s16 Y_LIMITS ;
static ssize_t touch_mode_store(struct class *cls, const char *_buf, size_t _count)
{
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        strncpy(temp, _buf + 5 * (j++), 4);
        uncali_limit_x = simple_strtol(temp, NULL, 10);
        strncpy(temp, _buf + 5 * (j++), 4);
        uncali_limit_y = simple_strtol(temp, NULL, 10);
		if((uncali_limit_x==-1)||(uncali_limit_y == -1))
				{
				uncali_limit_x =X_LIMITS;
		 		uncali_limit_y =Y_LIMITS;
				}
        printk("SN=%d uncali_x=%d uncali_y=%d\n", 
                i, uncali_limit_x, uncali_limit_y);
    return _count; 
}
...
static ssize_t touch_cali_status(struct class *cls, char *_buf)
{
    ...
    if (ret == 1){
		...
		X_LIMITS = uncali_limit_x;
		Y_LIMITS = uncali_limit_y;
