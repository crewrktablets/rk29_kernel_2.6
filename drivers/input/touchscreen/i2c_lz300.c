...
static s16 lz300msf_read_adc(struct i2c_client *client, s16 *pAdcArr)
{
  ...
  // loop max 3x until valid reading is received
  for (j = 3; ((z1 < Tth) || (z2 < z1)) && j; j--)
  {
  ...
  if (__gpio_get_value(LZ300MSF_IRQ) == 0 && z1 >= Tth)
  {
    ...
    if (pRatio < 12 && ((X_LIMITS - dx) >= TP_DEF_DX || (Y_LIMITS - dy) >= TP_DEF_DY))
    {
...
static void reportTouchPoint(struct lz300msf *ts, int id, int xAdc, int yAdc, int zAdc)
{
  ...
  input_report_abs(ts->input, ABS_PRESSURE,       zAdc ? 255 : 0);
  input_report_key(ts->input, BTN_TOUCH,          (zAdc > 0));
  if(zAdc)
  {
