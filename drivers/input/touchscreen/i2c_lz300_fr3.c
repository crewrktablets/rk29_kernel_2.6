...
#define DEBUG 0
#if DEBUG==1
#define DBG(format, ...) printk(format, ...)
#else
#define DBG(format, ...)
#endif
...
s16 lz300msf_read_adc(struct i2c_client *client, s16 *pAdcArr)
{
  ...
  do
  {
    ...
    DBG("LZ300-I2C=%02x%02x %02x%02x %02x%02x %02x%02x %02x%02x %02x%02x\n",
  ...
  DBG("LZ300-VAL=%04X %04X %04X %04X %04X %04X %04X\n",
