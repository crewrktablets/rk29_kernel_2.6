...
#define REPORT_BUFF_SIZE   3
static struct mma8452_axis axis_buf[REPORT_BUFF_SIZE];
...
static int mma8452_get_data(struct i2c_client *client)
{
	...
	mmaprintk("MMA8452-RAW: 0x%02x 0x%02x 0x%02x \n",buffer[0],buffer[1],buffer[2]);
	...
	// buffer readings
	axis_buf[reportCnt].x = axis.x;
    axis_buf[reportCnt].y = axis.y;
    axis_buf[reportCnt].z = axis.z;
    reportCnt++;
    reportCnt %= REPORT_BUFF_SIZE;

    // get average
    axis.x = (axis_buf[0].x + axis_buf[1].x + axis_buf[2].x /* + axis_buf[3].x*/) / REPORT_BUFF_SIZE;
    axis.y = (axis_buf[0].y + axis_buf[1].y + axis_buf[2].y /* + axis_buf[3].y*/) / REPORT_BUFF_SIZE;
    axis.z = (axis_buf[0].z + axis_buf[1].z + axis_buf[2].z /* + axis_buf[3].z*/) / REPORT_BUFF_SIZE;
