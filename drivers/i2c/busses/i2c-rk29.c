...
static int rk29_i2c_send_dir_msg(struct rk29_i2c_data *i2c, struct i2c_msg *msg)
{
	int i, ret = 0;
	unsigned long conr, lsr;
	...
	for(i = 0; i < 1; i++)
	{
		...
		if((ret = rk29_wait_event(i2c, RK29_EVENT_MTX_RCVD_ACK)) != 0)
			return ret;
		lsr = readl(i2c->regs + I2C_LSR);
		if((lsr & I2C_LSR_RCV_NAK) && (i != 1 -1) && !(msg->flags & I2C_M_IGNORE_NAK))
			return -EINVAL;
		udelay(i2c->udelay);
						 ...
						 struct i2c_msg *msg, int start, int stop)
{
	...
	{	
		if((ret = rk29_i2c_send_dir_msg(i2c, msg)) != 0)
		{
			rk29_set_nak(i2c);
