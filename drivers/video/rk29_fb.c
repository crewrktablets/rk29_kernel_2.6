...
void set_lcd_pin(struct platform_device *pdev, int enable)
{
	...
	fbprintk(">>>>>> lcd_standby(%d) = %d \n", lcd_standby, enable ? lcd_standby_pol : !lcd_standby_pol);
	 ...
	 inf= platform_get_drvdata(g_pdev);
        if(inf&&(inf->cur_screen == &inf->panel2_info)&&enable)
   	   return;
      }
    // set display_on
    if(enable)
...
static void open_backlight_timer(unsigned long data)
{
    gpio_set_value(RK29_PIN1_PB5, GPIO_LOW);
...
#endif
    setup_timer(&backlight_timer,open_backlight_timer, NULL);
    backlight_timer.expires=jiffies+msecs_to_jiffies(200);
