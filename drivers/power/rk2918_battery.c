...
#define BAT_2V5_VALUE	                3080
#define BATT_MAX_VOL_VALUE	  4165	//满电时的电池电压	 FOR A7
#define BATT_ZERO_VOL_VALUE       3450	//关机时的电池电压
...
static int rk2918_get_bat_capacity_ext(int BatVoltage)
{
    ...
    if (gBatChargeStatus == 1)
    {
        ...
        else if ((capacity <= gBatCapacity) && (80 <= gBatCapacity) && (gBatCapacity < 100))
        {
...
{		
	...
	if (rk29_battery_dbg_level)
	{
    	...
    	{
    	    AdcTestCnt = 0;
    	    printk("gBatStatus = %d, adc_val = %d, TrueBatVol = %d,gBatVol = %d, gBatCap = %d, captmp = %d, sec = %lu,ac=%d\n", 
    	            gBatStatus, AdcTestvalue, ((AdcTestvalue * BAT_2V5_VALUE * (BAT_PULL_UP_R + BAT_PULL_DOWN_R)) / (1024 * BAT_PULL_DOWN_R)), 
    	            gBatVoltage, gBatCapacity, capacitytmp, batteryspendcnt,gBatChargeStatus);
    	}
    }
...
static void rk2918_battery_work(struct work_struct *work)
{
    ...
    gBatteryData->dc_det_irq = gpio_to_irq(gBatteryData->dc_det_pin);
